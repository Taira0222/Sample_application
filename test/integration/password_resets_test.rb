...
class PasswordFormTest < PasswordResetForm
  ...
  test "reset with valid email" do # before_action valid_userのチェック
    assert_not_equal @user.reset_digest, @reset_user.reset_digest
    assert_equal 1, ActionMailer::Base.deliveries.size # メールが1通だけ送信されている確認
    assert_not flash.empty?
    assert_redirected_to root_url
  end

  test "reset with wrong email" do # before_action valid_userのチェック
    get edit_password_reset_path(@reset_user.reset_token, email: "")
    assert_redirected_to root_url
  end

  test "reset with inactive user" do # before_action valid_userのチェック
    @reset_user.toggle!(:activated) # activated をfalseに
    get edit_password_reset_path(@reset_user.reset_token, email: @reset_user.email)
    assert_redirected_to root_url
  end

  test "reset with right email but wrong token" do # before_action valid_userのチェック
...
class PasswordUpdateTest < PasswordResetForm
  ...
  test "update with valid password and confirmation" do
    ...
    assert is_logged_in?
    assert_not flash.empty?
    assert_nil @reset_user.reload.reset_digest # 演習p.675で追加
...
class ExpiredTokenTest < ExpiredToken
  ...
  test "should include the word'expired'on the password-resetpage" do
    follow_redirect!
